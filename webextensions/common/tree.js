        ...
        (async () => {
          ...
          if (options.duplicate) {
            ...
            const promisedDuplicatedTabs = Promise.all(apiTabIds.map(async (id, _aIndex) => {
              ...
              log(`ids from API responses are resolved in ${Date.now() - startTime}msec: `, aAPITabs.map(aPITab => aPITab.id));
              return aAPITabs;
            });
            if (configs.acceleratedTabDuplication) {
              ...
              const promisedDuplicatingTabs = (async () => {
                ...
                log(`ids from duplicating tabs are resolved in ${Date.now() - startTime}msec: `, aAPITabs.map(aPITab => aPITab.id));
                return aAPITabs;
              });
...
export async function openNewWindowFromTabs(tabs, options = {}) {
  ...
  browser.windows.get(newWindow.id, { populate: true })
    .then(aApiWindow => {
      ...
      const allTabsInWindow = aApiWindow.tabs.map(apiTab => TabIdFixer.fixTab(apiTab));
      const removeTabs = [];
...
function cleanUpTreeStructureArray(aTreeStructure, aDefaultParent) {
  ...
  aTreeStructure = aTreeStructure
    .map((aPosition, index) => {
      return (aPosition == index) ? -1 : aPosition ;
    })
    .map((aPosition, index) => {
      if (aPosition == -1) {
        offset = index;
  ...
  aTreeStructure = aTreeStructure.map(index => {
    return index < -1 ? aDefaultParent : index ;
  });
  return aTreeStructure;
}

export async function applyTreeStructureToTabs(tabs, aTreeStructure, options = {}) {
  if (!tabs || !aTreeStructure)
    return;

  MetricsData.add('applyTreeStructureToTabs: start');

  log('applyTreeStructureToTabs: ', tabs.map(dumpTab), aTreeStructure, options);
  tabs = tabs.slice(0, aTreeStructure.length);
  aTreeStructure = aTreeStructure.slice(0, tabs.length);
